...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const mpesaPhone = sanitize(body.mpesaPhone || body.customerPhone, 20)
    ...
    const items = body.items.slice(0, 50).map((item: Record<string, unknown>) => ({
    ...
    try {
      ...
      // swallow — the order is already saved
    }
    ...
    console.error("PayHero STK initiation failed:", error)
