-- Ankara Fashion Store - Database Improvements
-- Enhanced migration to improve existing tables
-- Created: 2026-02-12
...
-- Enhance PRODUCTS table with missing fields
...
-- Enhance PRODUCT_VARIATIONS table
ALTER TABLE product_variations ADD COLUMN IF NOT EXISTS attributes JSONB;

-- Add new CUSTOMERS table for better customer management
...
-- Add new DELIVERY_ZONES table (modernized delivery_locations)
...
-- Enhance NEWSLETTER_SUBSCRIBERS - add missing columns
ALTER TABLE newsletter_subscribers ADD COLUMN IF NOT EXISTS name VARCHAR(255);
ALTER TABLE newsletter_subscribers ADD COLUMN IF NOT EXISTS status VARCHAR(50) DEFAULT 'active';
ALTER TABLE newsletter_subscribers ADD COLUMN IF NOT EXISTS unsubscribed_at TIMESTAMP WITH TIME ZONE;

-- Enhance ORDERS table with new fields
...
-- Add new CUSTOMER_ADDRESSES table
...
-- ============================================================================
-- PHASE 3: ENHANCE HERO BANNERS TABLE
-- ============================================================================
...
-- ============================================================================
-- PHASE 4: ENHANCE ANALYTICS AND AUDIT
-- ============================================================================
...
-- ============================================================================
-- PHASE 5: AUTO-UPDATE TIMESTAMPS - CREATE OR REPLACE FUNCTION
-- ============================================================================
...
-- Apply/recreate triggers (safe - won't error if already exists)
...
DROP TRIGGER IF EXISTS update_product_variations_timestamp ON product_variations;
CREATE TRIGGER update_product_variations_timestamp BEFORE UPDATE ON product_variations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
DROP TRIGGER IF EXISTS update_delivery_locations_timestamp ON delivery_locations;
CREATE TRIGGER update_delivery_locations_timestamp BEFORE UPDATE ON delivery_locations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_newsletter_subscribers_timestamp BEFORE UPDATE ON newsletter_subscribers FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- ============================================================================
-- PHASE 6: SEED INITIAL DATA FOR ANKARA FASHION
-- ============================================================================

-- Insert Ankara Categories if they don't exist
INSERT INTO categories (name, slug, description, image_url, sort_order, is_active)
VALUES 
  ('Ankara Suits', 'ankara-suits', 'Premium ready-made Ankara suits for men and women', '/categories/suits.jpg', 1, true),
  ('Ankara Dresses', 'ankara-dresses', 'Beautiful Ankara dresses for all occasions', '/categories/dresses.jpg', 2, true),
  ('Ankara Shirts', 'ankara-shirts', 'Casual and formal Ankara shirts', '/categories/shirts.jpg', 3, true),
  ('Ankara Kimonos', 'ankara-kimonos', 'Trendy Ankara kimonos and loose wear', '/categories/kimonos.jpg', 4, true),
  ('Ankara Palazzo', 'ankara-palazzo', 'Comfortable Ankara palazzo pants', '/categories/palazzo.jpg', 5, true),
  ('Ankara Tops', 'ankara-tops', 'Stylish Ankara crop tops and blouses', '/categories/tops.jpg', 6, true),
  ('Ankara Sets', 'ankara-sets', 'Matching Ankara two-piece sets', '/categories/sets.jpg', 7, true),
  ('Ankara Accessories', 'ankara-accessories', 'Bags, headwraps, and jewelry', '/categories/accessories.jpg', 8, true)
...
VALUES
  ('store_name', 'Ankara Fashion', 'Store name', false),
  ('store_email', 'hello@ankarafashion.com', 'Support email', false),
  ('store_phone', '+254 700 000 000', 'Support phone', false),
  ('whatsapp_number', '+254 713 809 695', 'WhatsApp for orders', false),
  ('currency', 'KSh', 'Currency symbol', true),
  ('free_shipping_threshold', '5000', 'Free shipping minimum order', false),
  ('default_tax_rate', '0', 'Tax percentage', false),
  ('order_confirmation_email', 'Automatically send confirmation emails', 'Order management', false),
  ('enable_guest_checkout', 'true', 'Allow guest users to checkout', false),
  ('shop_status', 'open', 'Shop is open for orders', false)
...
-- Tables created/enhanced: 14+
-- New features: Enhanced product tracking, customer management, order shipments, analytics
